...
bool Box::IsCollidingWithRectangle(Box& p2, CollisionData& CollisionStruct)
{
    ...
    const Plane* plane_colliding = nullptr;
    std::vector<Vector3D> vertices_in_collision;

    //On parcours les sommets de l'objet A
    for(const Vector3D& vertex : vertices_A)
    {
        bool collision = true;

        //On parcours les faces de l'objet B
        for(const Plane& plane : planes_B)
        {
            const double interpenetration = plane.Normal.DotProduct(vertex.Sub(plane.Point));
            // si le sommet est dans la face
            ...
            if(collision)
            {
                if( abs(interpenetration) <CollisionStruct.PenetrationDepth)
                {
                    ...
                    vertices_in_collision.push_back(vertex);
    ...
    if(plane_colliding)
        CollisionStruct.CollisionNormal = plane_colliding->Normal;
